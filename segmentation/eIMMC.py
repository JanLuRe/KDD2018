...
class eIMMC:
    ...
    # @profile
    def fit(self, data, durations=None, key_reg=None, gt_display=None, eval_mode=False):
        self.data = data
        self.flat_data = [o for s in self.data for o in s]
        self.durations = durations if durations is not None else [np.ones(len(seq), dtype=int) for seq in self.data]
        ...
        if gt_display is not None: assert(len(gt_display) == self.nump)
        self.__rasterize_durations()
    ...
    def load_model(self, mc: ModelContainer):
        ...
        self.d_idx = mc.didx
        self.d_dim = 1 if self.d_idx is None else len(self.d_idx)
        self.dtheta = mc.dtheta
    ...
    def transform(self, X, Xd=[]):
        ...
        if len(Xd) > 0:
            ...
            Xd = [np.ones(len(session)) for session in X]
    ...
    def __perform_mcmc(self):
        ...
        for mcmc_iteration in range(self.cfg.num_it + 1):
            ...
            pb.printProgressBar(mcmc_iteration, self.cfg.num_it, prefix='Sampling: Progress', suffix='Complete', length=50)
    ...
    def __backward_step(self, msg, c_obs, c_dur):
        ...
        inter = np.multiply(self.__compute_intra((c_obs[0], -1), c_dur), np.dot(self.pi[:, :], sos).reshape(self.cfg.L, 1))
    ...
    def __compute_intra(self, c_obs, c_dur=[]):
        ...
        return np.multiply(self.psi[:, c_obs[0]].reshape(self.cfg.L, 1), theta).reshape(self.cfg.L, 1)
    ...
    def __init_params(self):
        ...
        self.psiMass = np.ones(self.L2 + 1) * self.cfg.sigma
        self.dSuper = np.ones(self.d_dim) * self.cfg.d_gamma

        # containers for parameters
        self.pi = np.zeros((self.cfg.L, self.cfg.L))
        self.psi = np.zeros((self.cfg.L, self.L2 + 1), dtype=np.float32)
        self.theta = np.zeros((self.cfg.L, self.L2 + 1, self.L2 + 1))
        self.dpsi = np.zeros((self.L2, self.d_dim))
        self.dtheta = np.zeros((self.cfg.L, self.L2, self.L2 + 1, self.d_dim))
        ...
        for cl in range(self.cfg.L):
            ...
            for nID in sel_ids:
                ...
                if not self.cfg.collapsed and nID < self.L2:
                    for cID in sel_ids:
    ...
    # @profile
    def __rasterize_durations(self, durdat=None):
        data = self.flat_durations if durdat is None else durdat
        d_model = bgm(self.cfg.k_gmm, max_iter=int(self.cfg.it_gmm))
        data = np.array(data).reshape(-1, 1)
        if len(data) / self.cfg.sub_data_size >= 2:
            data = shuffle(data, n_samples=int(self.cfg.sub_data_size))
        if self.cfg.collapsed:
            self.d_model = d_model
            self.d_idx = None
            self.d_dim = 1
            self.duration_vec = self.durations
            self.flat_duration_vec = np.array([[d] for d in self.flat_durations]).reshape(-1, 1)
        else:
            d_model.fit(data)
            pickle.dump(d_model, open('data/duration_model.pckl', 'wb'))
            d_idx = np.where(d_model.weight_concentration_[0] > self.cfg.d_importance_thresh)[0]
            d_dim = len(d_idx)

            print('Latent duration space dimensions: %i' % d_dim)
            if durdat is None:
                duration_vec = []
                flat_duration_vec = []
                for seq in self.durations:
                    seq = np.array(seq).reshape(-1, 1)
                    dvec_temp = d_model.predict_proba(seq)[:, d_idx].reshape(len(seq), d_dim)
                    dvec_temp = np.divide(dvec_temp, np.sum(dvec_temp, axis=1).reshape(-1, 1))
                    duration_vec.append(dvec_temp)
                    flat_duration_vec = dvec_temp if len(flat_duration_vec) == 0 else np.vstack((flat_duration_vec, dvec_temp))

                self.d_model = d_model
                self.d_idx = d_idx
                self.d_dim = d_dim
                self.duration_vec = duration_vec
                self.flat_duration_vec = flat_duration_vec
            else:
                return d_model, d_idx, d_dim
    ...
    def __init_vars(self):
        ...
        self.d = np.zeros((self.cfg.L, self.L2 + 1))  # for psi
        self.g = np.zeros((self.cfg.L, self.L2 + 1, self.L2 + 1), dtype=np.uint16)
        self.dg = np.zeros((self.cfg.L, self.L2 + 1, self.L2 + 1, self.d_dim))
    ...
    def __finalize_iteration(self, intra_step_count, mcmc_iteration, temp_assignments):
        ...
        if not self.eval_mode and mcmc_iteration in self.cfg.eval_ends:
            ...
            self.__save_result(tracks.astype(int))
    ...
    # # @profile
    # TODO: Add support for duration models
    # @profile
    def __save_result(self, tracks):
        for eval_set in tracks:
            tracking = self.cfg.assignment_counter[eval_set[0]]
            assignments = np.array([obs.argmax() for obs in tracking])
            final_super_state_ids = list(set(assignments))
            ids_to_final_ids = {oid: nid for nid, oid in enumerate(final_super_state_ids)}
            cfg = cp.deepcopy(self.cfg)
            cfg.L = len(final_super_state_ids)
            theta_count = np.zeros((cfg.L, self.L2 + 1, self.L2 + 1), dtype=np.float32)
            beta_count = np.zeros(cfg.L)
            psi_count = np.zeros((cfg.L, self.L2 + 1))
            pi_count = np.zeros((cfg.L, cfg.L))
            if not self.cfg.collapsed:
                # dpsi_count = [[] for dim in range(self.L2)]
                dtheta_count = np.zeros((cfg.L, self.L2, self.L2 + 1, self.d_dim))
            prev_c = ids_to_final_ids[assignments[0]]
            prev_p = self.data[0]
            ...
            for pos in range(1, self.nump):
                cur_c = ids_to_final_ids[assignments[pos]]
                assert(len(self.flat_data) == self.flat_duration_vec.shape[0])
                cur_p = self.flat_data[pos]
                cur_d = self.flat_duration_vec[pos, :]
                ...
                # end of a sequence
                    ...
                    dtheta_count[prev_c, prev_p, self.cfg.boundary, :] += cur_d
                else:
                    theta_count[cur_c, prev_p, cur_p] += 1
                    beta_count[cur_c] += 1
                    psi_count[cur_c, cur_p] += 1
                    if not self.cfg.collapsed: dtheta_count[prev_c, prev_p, cur_p, :] += cur_d
            ...
            theta_final = np.array(
                [np.divide(tc_row, np.sum(tc_row, axis=1).reshape(self.L2 + 1, 1)) for tc_row in theta_count])
            beta_final = beta_count / float(np.sum(beta_count))
            psi_final = np.divide(psi_count, np.sum(psi_count, axis=1).reshape(cfg.L, 1))
            pi_final = np.divide(pi_count, np.sum(pi_count, axis=1).reshape(cfg.L, 1))
            if not self.cfg.collapsed:
                dtheta_final = np.array(np.divide(dtheta_count, np.sum(dtheta_count, axis=3).reshape(cfg.L, self.L2, self.L2 + 1, 1)))

            dc = DataContainer(self.data, assignments, self.gt_display)
            if not self.cfg.collapsed:
                mc = ModelContainer(beta_final, pi_final, psi_final, theta_final, self.key_reg, self.L2, cfg, self.d_model, self.d_idx, dtheta_final)
            else:
                mc = ModelContainer(beta_final, pi_final, psi_final, theta_final, self.key_reg, self.L2, cfg)
            mc.cfg.assignment_counter = None
            mc.cfg.params_store = None
            mc.cfg.active_counters = None
            mc.cfg.elapsed_time = None
            save_path = self.cfg.log_path + self.cfg.eval_name + str(eval_set[0]) + '-' + str(eval_set[1]) + self.cfg.extension
            with open(save_path, 'wb') as f:
                pickle.dump(mc, f, protocol=4)
            save_path = self.cfg.log_path + self.cfg.assignments_name + str(eval_set[0]) + '-' + str(eval_set[1]) + self.cfg.extension
            with open(save_path, 'wb') as f:
